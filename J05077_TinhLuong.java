...
class TLuong {
    private String phongban, id, name;
    private int luongCoBan, soNgayCong;
    private Map<String, String> phongBanMap;

    public TLuong(Map<String, String> phongBanMap) {
    ...
    private void getPhongBan() {
        ...
        this.phongban = phongBanMap.get(maPhong); // hoặc dùng getOrDefault(maPhong, "Trả về giá trị theo ý muốn");
    }
    ...
    public String toString() {
        return id + " " + name + " " + phongban + " " + getTong();
    }
}
...
public class J05077_TinhLuong {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            String maPhong = in.next();
            String tenPhong = in.nextLine().trim();
            phongBanMap.put(maPhong, tenPhong);
        }

        int m = Integer.parseInt(in.nextLine());
        List<TLuong> list = new ArrayList<>();
        for (int i = 0; i < m; i++) {
            ...
            TLuong nhanVien = new TLuong(phongBanMap);
            nhanVien.setDuLieu(id, name, luongCoBan, soNgayCong);
            list.add(nhanVien);
        }
